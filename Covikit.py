...
try:
    ...
    q1 = st.selectbox("Cough issues?",("YES","NO")).lower()
    q3 = st.selectbox("Sore throat?",("YES","NO")).lower()

    
    q4 = st.selectbox("Shortness of breath?",("YES","NO")).lower()
    q5 = st.selectbox("Headache?",("YES","NO")).lower()


    
    q6 = st.selectbox("Chest pain or Pressure?",("YES","NO")).lower()
    q8 = st.text_input("Symptom period in days?")

    
    q2 = st.radio("Fever?",["Yes","No"]).lower()
    q7 = st.radio("Covid19 Identifier?",["None","Covid positive contact","Abroad Travel"]).lower()
